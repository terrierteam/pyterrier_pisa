...
class PisaIndex(pt.transformer.IterDictIndexerBase):
  ...
  def from_ciff(ciff_file: str, index_path, overwrite: bool = False, stemmer = PISA_INDEX_DEFAULTS['stemmer']):
    ...
    # TODO: Missing termlex file (opened issue)
    # TODO: Stemming, the user must provide a matching stemmer or perform it as a pre-processing step. Should emit a warning to this effect.
    with open(ppath/'pt_pisa_config.json', 'wt') as fout:
      json.dump({
